...
describe('End to end test ', () => {
    it('visits the app root url', () => {
      ...
      cy.get('.answer').contains(/^[1][0]$/).should('be.visible');
      cy.get('.answer').contains(/^[1][0]$/).filter(':visible').click();
      cy.get('.question').contains('What is 5 * 5 ?').should('be.visible');
      cy.get('.answer').contains(/^[2][5]$/).should('be.visible');
      cy.get('.answer').contains(/^[2][5]$/).filter(':visible').click();
      cy.get('.question').contains('What is 5 / 5 ?').should('be.visible');
      ...
      cy.get('.desc').contains('Studying has definitely paid off for you!').should('be.visible');
      // redo the quizz
      ...
      cy.get('.answer').contains(/^[5][5]$/).should('be.visible');
      cy.get('.answer').contains(/^[5][5]$/).filter(':visible').click();
      cy.get('.answer').contains(/^[1][5]$/).should('be.visible');;
      cy.get('.answer').contains(/^[1][5]$/).filter(':visible').click();
      // change quizz
